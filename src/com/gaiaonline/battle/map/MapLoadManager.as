package com.gaiaonline.battle.map
{
	...
	public class MapLoadManager extends EventDispatcher implements IAreaLoadingHandler, IMapSoundLoaderHandler
	{
		...
		///---- Load Dependencies  (for Shared Libs)
		...
		public function startLoad(zone:String):void{
			...
			//we don't use this random stuff at all. sigh.
			var img:String = "";
		...
		private function loadSounds(fileName:String):void{			
			...
			var loadMapSoundsUrl:String = this._baseUrl + "sounds/" + fileName.toLowerCase() + "_sounds.swf";
		...
		private function loadMainSwf():void{
			...
			mapLoader.contentLoaderInfo.addEventListener(Event.COMPLETE, onMapLoaded)//,false, 0,true);
		...
		private function tryToDispatchCompleteEvent():void{
			...
			//the server is still loading/compiling scripts.  Kinda rare for the client to finish first, but it'll be helpful to show what's going on in the preloader.
