package com.gaiaonline.battle.map
{
	...
	public class TalkIconManager implements ITalkIconManager{
		...
		private var _talkIcons:Dictionary = new Dictionary(true);
		private var _invalidActors:Dictionary = new Dictionary(true);
		private var _frameTimer:FrameTimer = new FrameTimer(onFrame);
		private var talkIconStopper:DisplayObjectStopper = new DisplayObjectStopper(DisplayObjectStopperModes.SHOW_NO_ANIM, true);
		...
		private function clear():void{
			for each (var talkIconObj:Object in _talkIcons){
				var talkIcon:TalkIcon = talkIconObj as TalkIcon;
				talkIcon.dispose();
			}
		...
		private function addDialogBtn(event:GlobalEvent):void {
			...
			// if a dialog button already exists for that actor, reuse it 
			...
			if(_talkIcons[actor]){
				...
				var talkIcon:TalkIcon = new TalkIcon(_talkIconLayer,priority);
		...
		private function getActorFromTalkIcon(talkIcon:TalkIcon):BaseActor{
			var actor:BaseActor;
			for (var actorObj:Object in _talkIcons){
		...
		public function removeDialog(dispObj:DisplayObject):void {
			...
			if(actor){
				var talkIcon:TalkIcon = _talkIcons[actor];
		...
		private function onFrame():void{
			...
			for (var actorObj:Object in _invalidActors){
				var actor:BaseActor = actorObj as BaseActor;
