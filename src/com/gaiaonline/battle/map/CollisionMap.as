...
{
	// [kja] NOTE: we're using all 8 bits of the byte.  Adding any more options would require changing our data representation.  
	public static const SLOT_NULL:uint			= 0;
	public static const TYPE_OPEN:uint			= 0x01;
	public static const TYPE_WALL:uint			= 0x02;
	public static const TYPE_LOW_WALL:uint		= 0x04;
	public static const TYPE_WATER:uint			= 0x08;
	public static const TYPE_DEEP_WATER:uint	= 0x10;
	public static const TYPE_SAND:uint			= 0x20;
	public static const TYPE_NOFLYZONE:uint		= 0x40;
	public static const TYPE_BITMASK:uint  		= 0x7f;
	public static const TRIGGER_BITMASK:uint	= 0x80;

	static private var _charCodeToSlot:Array;
	private static function initDeserializationLookup():void
	{
		// [kja] dumb, but fast, simple, efficient
		_charCodeToSlot = [];
	...
	private var scale:Number = 75; // int, percentage representing the "zoom factor" of this room. Typical rooms are at 75, or 75%.
	private var scaleMult:Number;
	private var resolution:Number = 8; // the resolution of the map data
	...
	private var map:ByteArray = new ByteArray();
	...
	{
		const slot:uint = map[getNodeIndex(mp.getMapX(), mp.getMapY())];
		const type:uint = slot & TYPE_BITMASK;
	...
	{
		const index:Number = getNodeIndex(x, y);
		if (index < map.length)
		{
			const slot:uint = this.map[index];
	...
	public function draw(mc:Sprite):void
	{
		...
		{			
			mc.graphics.lineStyle(0, 0, 0); // no line
			mc.graphics.beginFill(0x0000FF, 0.4);			
		...
		for (i = 0; i < numCols; i++)
		{
			...
			{
				var slot:uint = map[getNodeIndex(i, j)];
				var type:uint = slot & TYPE_BITMASK;
				
				if (type && type != TYPE_OPEN){
					
					var xx:int = i*this.resolution;
					var yy:int = j*this.resolution;
					
					if (type != TYPE_OPEN){
						mc.graphics.lineStyle(1,0xAA0000, 0.5);
						mc.graphics.beginFill(0xAA0000, 0);
					...
					if ((slot & TRIGGER_BITMASK) != 0){						
						...
						mc.graphics.beginFill(0x00FF00, 0);
