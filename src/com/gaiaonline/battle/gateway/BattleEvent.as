package com.gaiaonline.battle.gateway
{
	...
	{
		// [bgh] zomg_login is the event that is returned from the
		// battle (java) server when a session ID is verified.
		...
		// [bgh] The login constants below are specific to the flash
		// client authenticating with GSI (php) direcly.
		...
		{
			//trace("[BEVT] new BattleEvent( "+type+", "+msg+" )");
			super(type, false, false);
			this.myMessage = msg;
			//if( myMessage != null ) { myMessage.debugTrace(); }
			//trace("[BEVT]   :)");
		}
