package com.gaiaonline.battle.newactors
{
	...
	public class EnvActor {
		...
		private function onUse(evt:UiEvents):void{
			/*		
			var playerPos:Point = ActorManager.getInstance().myActor.position;
			var dx:Number = this.position.x - playerPos.x;
			var dy:Number = this.position.y - playerPos.y;
			var dist:Number = Math.sqrt(dx*dx + dy*dy);		
			trace(dist);
			*/
			//trace("[EnvActor] On Use", evt.value);
		...
		public function onFrameTimer():void {
			if (!this._usableStateValid) {
				var mc:Sprite = this.mObj.getEventMc();
