package com.gaiaonline.battle.ui.battlewin
{
	...
	public class BattleWinMsg extends MovieClip
	{
		...
		private var _mesages:Dictionary = new Dictionary(true);
		private var _index:int = 0;
		private var _lastIndex:int = 0;
		private var _btnClose:SimpleButton;
		private var _itemContainersScrollRect:Rectangle = new Rectangle(0,0, 350, 150); 
		...
		{	
			var item:BattleWinMsgItem;
			if (show){
				if (this._mesages[name] == null){	
					var c:Class = this.loaderInfo.applicationDomain.getDefinition("msgBoxItem") as Class;				
					item = new c();
					//item.x = 20;					
		...
		private function removeItem(name:String):void{
			if (this._mesages[name] != null){
				var item:BattleWinMsgItem = this._mesages[name];
		...
		private function setOpen(openItem:BattleWinMsgItem):void{
			for each(var item:BattleWinMsgItem in this._mesages){				
				if (item != openItem){
					item.open = false;
			}else{
					//item.open = true;			
		...
		private function arange():void{
			...
			for (var i:int =0; i < _itemList.length; i++){				
				var sortedItem:BattleWinMsgItem = _itemList[i];				
				sortedItem.y = yp;
				if (sortedItem.open){
					yp += sortedItem.height -1;
				}else{
					yp += 20;							
				}										
			}	
		...
		private function onFrame():void{
			if (this._open && this._itemContainersScrollRect.y != 0){						
				var rect:Rectangle = this._itemContainersScrollRect;
				rect.y -= this._itemContainersScrollRect.y/4;				
				if ( (rect.y/4) < 1){
					rect.y = 0;
				} 
				this.setItemsContainerScrollRect(rect);
			}else if (!this._open && this._itemsContainer.numChildren > 0 && this._itemContainersScrollRect.y < this._itemsContainer.height + 5){														
				var rect2:Rectangle = this._itemContainersScrollRect;
				rect2.y += ((this._itemsContainer.height+5) - this._itemContainersScrollRect.y)/4;				
