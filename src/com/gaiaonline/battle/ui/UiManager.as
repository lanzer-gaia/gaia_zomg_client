package com.gaiaonline.battle.ui
{
	...
	public class UiManager extends EventDispatcher 
		implements IShopWidgetEventHandler, 
				   IAsyncCreationHandler, 
				   IActorRelocateHandler, 
				   IMapFilesLoadHandler,
				   IEmoteEventHandler,
				   IRegisterHandler
	{
		...
		private var mcWinLayer:Sprite;  //KAI: this should go away when our conversion to Flex is complete	
		...
		// view controls/managers
		private var _configManger:ConfigManager = ConfigManager.getInstance();
		...
		// windows
		// [kja] these window types are temporarily untyped while we deal with MovieClip -> Flex conversion issues
		private var winFrameRate:*;
		//private var winAdmin:Window;
		...
		// playerStats (with paperDolls inside)
		...
		private var _moveDir:Number = UserInput.NULL_MOVEDIR;
		...
		public function UiManager(gatewayFactory:IGatewayFactory, 
								  uiFramework:IUIFramework, 
								  linkManager:ILinkManager, 
								  gst:GST, 
								  fileVersionManager:IFileVersionManager, 
								  userSettings:UserSettings, 
								  emoteManager:EmoteManagerOld, 
								  actorManager:ActorManager, 
								  flexUIManager:IFlexUiManager,
								  gameWindowFactory:IGameWindowFactory, 

								  partnerId:String, 
								  isLoadedExternally:Boolean, 
								  useCaptcha:Boolean,

								  flexShopWidgetLayer:DisplayObjectContainer, 
								  mcBaseWinLayer:Sprite, 
								  mcWinLayer:Sprite,
								  flexWinLayer:DisplayObjectContainer,
								  mcDragLayer:Sprite, 
								  mcLoadLayer:Sprite, 
								  mcToolTipLayer:Sprite, 
								  dummyLayer:MovieClip, 
								  errLayer:Sprite,
								  progressManager:ProgressEventManager)
		{
			...
			DisplayObjectUtils.addWeakListener(GlobalEvent.eventDispatcher, GlobalEvent.WORLD_COUNTER_UPDATE, onWorldCounterUpdate);																					// World Events
		...
		// [kejk] ugly but necessary for now
		...
		private function onMapServerZoneProgress(event:ProgressEvent):void{
			//setPreloader(event.bytesLoaded, "Loading Scripts");
		}
		...
		private function onUserLevelSet(evt:GlobalEvent):void {
			if (evt.data.isGuest) {
				// set up the annoying prompt
				if (this.timerGuest == null) {
					this.timerGuest = new Timer(1000 * 60 * 20);		// every 20 minutes - only after they finish a quest
					this.timerGuest.addEventListener(TimerEvent.TIMER, onRegisterPrompt, false, 0, true);
				}
			} else {
				if (this.timerGuest != null) {	// was a guest user and now no longer is, remove guest ui treatment
					this.timerGuest.stop();
				}
			}
			
			///--- add AdminPanel to menu
			if (_actorManager.myActor.isDev()) {
				if (this._uiFramework.showFrameRate) {
					// Create FrameRate Panel
					const showNetworkStats:Boolean = true;
					const panelHeight:Number = 120 + (showNetworkStats ? 25 : 0);
					
					this.frameRate = new FrameRate(_uiFramework, this._gateway, showNetworkStats);
					if (this._fileVersionManager) {
						var clientVersion:String = this._fileVersionManager.getClientVersion(null); 
						if (clientVersion != null) {
							this.frameRate.setClientVer(clientVersion);
						}
						var serverVersion:String = this._fileVersionManager.getServerVersion();
		...
		//**** Load / Init UI && windows ******************
		//** - LoadPreloader
		//** - Init Prelaoder
		//** - Load uiloadmanager.swf		
		//** - Load Action Bar 
		//** - init Action Bar 
		//** - init Panel and Windows (loginScreen, frameRate, AdminPanel)
		...
		// Init Preloader - Load uimanager.swf
		...
		private function loadUiManager():void{
			...
			//mapFiles.load(this._linkManager.baseURL);
		}
		...
		private function onUiCoreComplete(event:Event):void{
			...
			if (this._linkManager.spaceName == "zomg"){
				var uiManager2Version:String = "?v=" + this._fileVersionManager.getClientVersion("ui/uimanager2.swf");			
		...
		private function init():void{
			...
			if (this._uiFramework.loadUi) {
				// preload the a module so the module infrastructure is in place; might as well load the actor info one
				this._chatController = ChatController.getInstance(this._flexUIManager, this._gateway, this._emoteManager, this, this);
				var register:RegisteredClasses;	//[kejk] make this a module?
				
				loadModules();
								
				/* TODO: remove this? once flex version works?
				// Admin Panel
				this.adminPanel = this.getNewAdminPanel();			
				this.adminPanel.addEventListener("Send", onAdminPanelSendClick, false, 0, true);
				this.adminPanel.addEventListener("ShowCollision", onAdminPanelShowCollisionClick, false, 0, true);
				this.adminPanel.addEventListener("ChkSpawnClick", onAdminPanelChkSpawnClick, false, 0, true);
				this.adminPanel.addEventListener("SetGst", onAdminPanelSetGSTClick, false, 0, true);
				this.adminPanel.addEventListener("Lights", onAdminPanelLightsClick, false, 0, true);
				
				this.winAdmin = new Window(this._linkManager.baseURL + this._configManger.genericWindowUrl, this.adminPanel, true, false, true, false, 420, 205, 420, 205, false, false, this._uiFramework.loaderContextFactory);
				this.winAdmin.addEventListener("CLOSE", onAdminPanelClose, false, 0, true);	
				this.winAdmin.useMask(false);
				BattleUtils.disableMouseOnChildren(this.winAdmin);
				*/
			...
			// --- Play Movie
			// This is tricky; right now, this can be dispatched via BattleGateway's processDispatchCmd, which you won't
			// find it via grepping.
		...
		private function initUserLayout():void{
			...
			if (this.winPDAPanel) {
				...
				// [kja] this "Full"/"Normal" mode stuff is now no longer in use.  It should get stripped away next time we're changing the settings stuff.
			...
			// winMap default setting are in BattleWin.as
			// actionBar defautl setting are in UiActionBar.as
			// chatPanel defautl Setting are in UiChatPanel.as
			...
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.CHAT_PANE_OPEN, {leadingText:null, setFocus:false}));
...
// TODO: support flex version.
//				if (this.winAdmin) {
//					this._userSettings.addSetting("winAdmin", this.winAdmin);
//				}
		...
		private var _uiState:WidgetOpenStates = new WidgetOpenStates();
		...
		private function onWorldTimerUpdate(evt:GlobalEvent):void {
			...
			const countUp:Boolean = startTime.getTime() < finishTime.getTime();
			...
			switch (widgetName) {
				...
				case UiTesterController.NEW_RECREWT:
					if (show)
					{
						...
						{
							var obj:DisplayObject = this._windowFactory.createWindow(widgetName, true);
		...
		private function onChatFiltersLoaded(evt:BattleEvent):void{
			//[Fred] this is not used anymore (noto sure it ever worked
			/*
			var responseObj:Object = evt.battleMessage.responseObj[0].values;
			if (responseObj && this._chatController) {		
				this._chatController.setChatFilters(responseObj);
			}
			*/
		}	
		...
		private function onServerListHelpLink(e:TextEvent):void {
			...
			switch(cmd.toLowerCase()){
				case "close":  
					var footer:SvrHelpFooter = this.wslHelpDialog.footer as SvrHelpFooter;
		...
		private function onRegister(evt:UiEvents):void {
			onRegPromptClose();	// just in case - don't reset timer, if they sign back in as user, that will be handled automatically
		...
		private function onRegistrationLoaded(e:Event):void
		{
			...
			const REG_WIDTH:uint = this._useCaptcha ? 618 : 480;
			const REG_HEIGHT:uint = 365;
			this.winRegister = new Window(
				this._linkManager.baseURL + this._configManger.genericWindowUrl, 
				registration, 
				true, 
				false,
				false, 
				...
				false, 
				false, 
				this._uiFramework.loaderContextFactory
			);
		...
		private function onRegComplete(e:Event):void
		{
			...
			var regCompleteTrackingURLs:Array =  // these will be un-hardcoded, shortly.  -kja 
			[
				"http://media.fastclick.net/w/roitrack.cgi?aid=7366",
				"http://network.realmedia.com/RealMedia/ads/adstream_nx.ads/TRACK_Gaiaonline/Trackit_Confirmationpage_Nonsecure@Bottom3",
				"http://server.cpmstar.com/action.aspx?advertiserid=83&gif=1"
			];					

			for each (var url:String in regCompleteTrackingURLs)
			{
				StepLoader.add(new Loader(), new URLRequest(url), this._uiFramework.loaderContextFactory.getLoaderContext());
			}

			//
			// We could fire an event here, but we'll wait for the server to send us an actor update.
			// When that actor update arrives, we'll detect ourselves if it's a guest registration success.
			// Not the best way to do it (ideally the server would handle all the state for us), 
			// but at least it consolidates the event -kja 
		...
		private function onShopWidgetLoaded(instance:Object):void {
			...
			shopWidgetImpl.init(null, // right now, the zomg server is pulling shop data from more than one shop 
								this._linkManager.getLink("gcash_home"), 
								this._gateway, this._gateway, this._gateway, 
								this._gCash);
		...
		public function onShopWidgetNPCDialogue(message:String, npcName:String):void {
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.CHATTABLE_MSG, { msg:message, type:"dialog", received:true, fromSystem:false, name:npcName}));			
		}
		...
		private function onShopWidgetAcceptClick(event:Event):void
		{
			...
			{
				// [bgh] removed javascript reference to vivox code
			}	
		}
		...
		public function showShopWidget(show:Boolean, source:DisplayObject = null):void {	
			...
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.BACK_ALLEY_BARGAINS_SHOW_CHANGED, {showing:show, source:source}));
		}
		...
		private function positionShopWidget():void {
			...
			// using raw member variable for shop widget, since I don't want to force a creation too soon, just because the map is resizing on startup
			if (this._shopWidgetInstance && this.winMap && this._flexShopWidgetLayer) {
				...
				(this._shopWidgetInstance as IShopWidget).setTabGap(this._shopWidgetInstance.width - 20 - this.winMap.getShopButtonWidth() + 2, 
														this.winMap.getShopButtonWidth()-2);
			}																						
		}		

		// Ugh.  Looks like we need a bit of a delay here because the map doesn't finish resizing until a frame.
		private var _winMapResizeTimer:FrameTimer = new FrameTimer(onWinMapResizeLater);
		...
		private function showShopCallout(show:Boolean):void {
			...
			if (!show) {
				...
				var shopCalloutShown:Object = {"shopCalloutShown":true};
				var msg:BattleMessage = new BattleMessage("putNkvp", shopCalloutShown);
				this._gateway.sendMsg(msg);					
			}
		}		
		...
		private function getCallOutSettings():void {
			if (this._gateway.connectedToGameServer) {
				var msg:BattleMessage = new BattleMessage("getNkvp", {keys:["shopCalloutShown"]});
		...
		// after guest leaves the sewers
		public function showFirstRegisterPrompt(widgetPromptTitle:String, widgetPromptContent:String):void {
			if (ActorManager.getInstance().myActor.isGuestUser()) {
				var title:String = widgetPromptTitle;
				var body:String = widgetPromptContent;
				showRegistrationPrompt(title, body, "prompt1");
				if (timerGuest != null) {
					this.timerGuest.start(); // now start bugging them :)
				}
			}
		}					
		
		// -- sit / Stand
		private  var sitStandTimer:Timer = new Timer(SIT_STAND_DELAY_TIME,1);
		...
		private function onChatEmote(evt:BattleEvent):void{
			for (var i:int = 0; i < evt.battleMessage.responseObj.length; i++){
				...
				if (obj.emoticonID != null && obj.senderID != null){
					var ba:BaseActor = ActorManager.actorIdToActor(obj.senderID);
		...
		public function onEmotePopupChange(shown:Boolean):void {
			const type:AlertTypes = shown ? AlertTypes.GENERIC_OPEN : AlertTypes.GENERIC_CLOSE;
		...
		private function playEmote(emoticonID:String):void {
			...
			if (this._uiFramework.map != null && this._uiFramework.map.isMapLoaded && (now - lastEmote) > 2000){
				...
				var msg:BattleMessage = new BattleMessage("showEmoticon", obj);
				this._gateway.sendMsg(msg);				
				this.lastEmote = now;
			};
		}		
		...
		/* TODO: move to flex implementation.
		private function onAdminPanelSendClick(evt:Event):void{
			var obj:Object = this.adminPanel.getCall();
			var params:Array = String(obj.param).split(",");
			
			if (!isNaN( parseInt(obj.cmd)) ){
				var msgOld:BattleMessage = new BattleMessage(obj.cmd, params);		
				msgOld.addEventListener(BattleEvent.CALL_BACK, onAdminCommadCallBack);
				this._gateway.sendMsg(msgOld);	
			}else{
				...
				var msgNew:BattleMessage = new BattleMessage(obj.cmd, paramObj);
				msgNew.addEventListener(BattleEvent.CALL_BACK, onAdminCommadCallBack);
				this._gateway.sendMsg(msgNew);								
			}
			
		}	
		
		private function onAdminCommadCallBack(evt:BattleEvent):void{
			this.adminPanel.setCallbackObject(evt.battleMessage.responseObj);			
			this._gateway.handlePushMessage(evt.battleMessage);	
			BattleMessage(evt.target).removeEventListener(BattleEvent.CALL_BACK, onAdminCommadCallBack);	
		}		
				
		private function onAdminPanelClose(evt:Event):void{
			if (this.mcToolTipLayer.contains(this.winAdmin)){
				GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ALERT, {type:AlertTypes.GENERIC_CLOSE}));
				this.mcToolTipLayer.removeChild(this.winAdmin);
			}
		}	
		
		private function onAdminPanelShowCollisionClick(evt:Event):void{
			if (!this._uiFramework.map.isCollisionShowing()){	
				this._uiFramework.map.drawCollisionMap();
				this._uiFramework.map.setCollisionShowing(true);
			}else{
				this._uiFramework.map.setCollisionShowing(false);
			}
		}
				
		private function onAdminPanelChkSpawnClick(evt:UiEvents):void{
			this.showSpawners = evt.value;			
		}
		
		private function onAdminPanelSetGSTClick(evt:UiEvents):void{
			//trace("Set GST ", evt.value.h, evt.value.m)
			var now:Date = new Date();
			var d:Date = new Date(now.fullYear, now.month, now.date, evt.value.h, evt.value.m);
			this._gst.time = d;
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.MAP_SET_LIGHTS, {on:this._gst.lightsOn}));
		}
		private function onAdminPanelLightsClick(evt:Event):void{
			var l:Boolean = !this._uiFramework.map.isLightsOn();
		...
		*/		
		...
		// logout priority = HIGH  // this gets precedence 
		// other priority = MEDIUM
		// timer priority = LOW
		public function showRegistrationPrompt(txtTitle:String, txtBody:String, origin:String, priority:uint=LOW):MovieClip {
			if (regPromptShowing()) {
				if (priority != 0) {
					onRegPromptClose();		// only 1 at a time but everything but the 30 minute prompt takes priority
				} else {
			...
			this.winRegister.x = (origin == "prompt1") ? (this.mcWinLayer.stage.stageWidth - rp.height)/4 : (this.mcWinLayer.stage.stageWidth - rp.height)/2;
			this.winRegister.y = (origin == "prompt1") ? (this.mcWinLayer.stage.stageHeight - rp.height)/4 : (this.mcWinLayer.stage.stageHeight - rp.height)/2;
			this.mcToolTipLayer.addChild(this.winRegister);
			this.winRegister.addEventListener("CLOSE", onRegPromptClose, false, 0, true);
			if (isLogout) {
				rp.addEventListener("QuitClick", onRegPromptLogout);	// logout
			}
			rp.addEventListener(UiEvents.REGISTER, onRegister);				// centralize this here
		...
		private function showMembersOnlyMessage(): void {
			...
			{
				var membersOnlyMessage: MovieClip = new MembersOnlyMessage();
		...
		{
			//[kja] this is temporary - the accept/decline dialog needs to be replaced
		...
		private function onGstLoaded(evt:GlobalEvent):void{
			...
			//this.setPreloader(90, "Loading Character Info: Quest info");
			if (this.pdaWidget) {		
		...
		private function onQuestsLoaded(evt:GlobalEvent):void{
			...
			//this.setPreloader(100, "Loading Character Info");
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.PLAYER_INFO_LOADED, {}));
		}

		private function onCollectiblesLoaded(e:GlobalEvent):void {		
			// if the collectiblesManager exists, then the collectibles UI is open already, so let's refresh it right now
		...
		private function createOptionPanel():void {
			...
			obj.tabs[0] = {
				...
				controls:{
					typ:"volume",
					data:[
						  {title:"MUSIC", event:UiEvents.SET_MUSIC_VOLUME, _defVol:this._uiFramework.volumes.musicVolume, addPlayerLink: true},
						  {title:"SOUND", event:UiEvents.SET_SFX_VOLUME, _defVol:this._uiFramework.volumes.soundVolume}
						  ]
					}
				};
			obj.tabs[1] = {
		...
		private function showMinimizedMP(e:MPEvents):void{
			...
			DisplayObjectUtils.bringToFront(this.miniMusicPlayer); //[kja] we should really be doing this for all children on this layer.  Flex would give this for free, or at least make it easier.
		}
		...
		{
			const dobj:DisplayObject = DisplayObject(e.target);
		...
		private function onDialogDataUpdated(e:GlobalEvent):void {
			...
			if (obj.dlg == "DONE" && obj.con != null){
				//trace("===========  END CONV", obj.con)
				var isNaturalDone:Boolean = true;
				if (obj.isInfo) {
			...
			if (obj.dlg != null && obj.con != null && obj.npc != null){
				var ac:BaseActor  = ActorManager.actorIdToActor(obj.npc);										
				if (ac != null && _actorManager.myActor != null){
					...
					if (obj.playerText != null || obj.options != null ){
						...
						if (obj.options != null){
							options = new Array();
							for (var oid:int = 0; oid < obj.options.length; oid++){
								options.push({id:oid, txt:obj.options[oid]});							
							}
						}
					...
					//convId:String, dialogId:String, npcId:String, actorId:String, actorName:String, actorMc:Sprite, txt:String = null, options:Array = null								
				}				
			}			
		}
		...
		private function onDialogPanelResponse(evt:UiEvents):void{
			...
			if (evt.value.numberOfTimesSent < 2) { // we increment this value when we resend the request to the server; should be 1 or greater
		...
		private function showPlayerStats(actor:BaseActor):void{
			...
			if (this.playerStats[actor.actorId] == null){	
				var showAllTabs:Boolean = (actor.actorId == ActorManager.getInstance().myActor.actorId);						
				var ps:UiPlayerStats = new UiPlayerStats(this._uiFramework, this._linkManager, showAllTabs);
				
				if (actor == _actorManager.myActor){	
					ps.ghiProgess = _actorManager.myActor.ghiProgress;
				}
				 			
				var w:* = this._windowFactory.createLegacyZomgWindow(this._linkManager.baseURL + this._configManger.genericWindowUrl, ps, true, false, true, false, 290, 400, 290, 400, false, false, this._uiFramework.loaderContextFactory);
				w.useMask(false);				
				...
				obj.userName = actor.actorName // String User Name
				...
				//--- call for paperdoll info; first set up state and listeners
				...
				var pd:UiPaperDoll = ps.paperDoll;
				var param:Object  = {id:actor.actorId};
				param = {id:actor.actorId};
				var msg:BattleMessage = new BattleMessage("subscribeToPaperDollInfo", param);
				msg.tag = pd;
				this._gateway.sendMsg(msg);

				// if the actor gets removed, we need to cancel our subscription, close the window, etc.;
				// this will happen if the actor warps away from us, etc.
				// Note that when we warp, this event doesn't fire.
		...
		private function onActorRemovingFromActorList(e:Event):void {
			// let's close the player stats window, since we can no longer really subscribe to updates 
			// for this actor (plus, it's preferred for game play)
			// Note that the close handler will unsubscribe us from player updates, etc.
			...
			if (actor != null) {
				...
				// simulate a close
				var w:* = this.playerStats[actor.actorId];
				if (w != null) {
		...
		private function onPaperDollCallBack(evt:BattleEvent):void{
			if(evt.battleMessage.tag != null && evt.battleMessage.responseObj != null){
				var rObj:Object = evt.battleMessage.responseObj[0];
		...
		private function processPaperDollCallBackObj(rObj:Object):void {
			var obj:Object = new Object();
			obj.userName = rObj.name // String User Name
			obj.clanName = rObj.clanName || " ";

			if (rObj.conLevel) {
				var conLevelNum:Number = rObj.conLevel;
			...
			if (rObj.ringInfoList != null) {
				obj.rings = new Array();
				for (var ri:int = 0; ri < rObj.ringInfoList.length; ri++){
					obj.rings.push( {ringId:rObj.ringInfoList[ri].id, url:rObj.ringInfoList[ri].url, name:rObj.ringInfoList[ri].name,description:rObj.ringInfoList[ri].description} ) 
				}				
			} else {
		...
		private function unsubscribeToPaperDollUpdates(actor:BaseActor):void {
			if (actor != null) {
				...
				var actorId:String = actor.actorId;								
				var param:Object  = {id:actorId};
				if (actorId == ActorManager.getInstance().myActor.actorId) {
					var msg:BattleMessage = new BattleMessage("unsubscribeToPaperDollInfo", param);
					this._gateway.sendMsg(msg);					
				}
			}
		}
		...
		private function setTargetOpenState(target:BaseActor):void {
			...
			if (target != null){
				if (this.actionBar == null || (this.actionBar && !this.actionBar.usePowerMeter)) {	/// using regular selection (not using power meter)					
					GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ACTOR_INFO_OPEN, null));	
					return;
				}else{ //---- Using power meter				
					RingItemManager.getInstance().selectedSlot = -1;	// unselect any selected ring
				}				
			} 
		...
		private function openYouAreDazed():void{			
			...
			if (!isWindowOpen(this.winYouAreDazed) ){
				...
				var size:Object = this.winYouAreDazed.getSize();
		...
		private function onYouAreDazeGotoClick(evt:UiEvents):void{
			if (isWindowOpen(this.winYouAreDazed)){
				var msg:BattleMessage = new BattleMessage("awaken", {gtfo:true});
				this._gateway.sendMsg(msg);				
				closeYouAreDazed();
			}			
		}
		...
		private function onReportAbuse(evt:GlobalEvent):void {
			var data:Object = evt.data;
			if (data != null) {
				var sessionId:String = this._gateway.sessionId;
				var reportType:String = data.reportType;
				var offenderName:String = data.offenderName;
				var reportNotes:String = data.reportNotes || "";
				if (sessionId && reportType && offenderName) {
					var chatLog:String = this._chatController.getAbuseLog();					
		...
		public function playMovie(url:String, width:int = 100, height:int = 100, xPos:int = -1, yPos:int = -1, hideNpcDialog:Boolean = false):void{			
			...
			///---- replace the [space] with spaces/spaceName ....  
			...
			if (xPos < 0 || yPos < 0){
				var mapSize:Object = this.winMap.getSize();
				if (xPos < 0){				
		...
		private function onMovieDone(evt:Event):void{
			...
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.MOVIE_DONE, this.uiMoviePlayer.url)); // [kja] the server dynamically registers a listener for this - script uses it
		...
		private function initCenterPrintMap():void {
			...
			// this._centerPrintMap[AlertTypes.GHI_ABILITY_EARNED.toString()] = new CenterPrintData(GlobalTexts.getNewGhiCenterPrint,CenterPrintMsg.RING_TYPE);
		}

		private function onAlertEvent(e:GlobalEvent):void 
		{
			const data:Object = e.data;
			const type:AlertTypes = data.type;
			
			var soundId:String = this._soundMap[type];
			if (type == AlertTypes.ITEM_SPECIFIC)
			{
				const extra:String = data.extraId || "";
				soundId = "item_" + extra;
				} 
			if (soundId)
			 {
			...
			if (centerPrintData != null) {
				var textFnArgs:Array = new Array();
				var textParam:String = data.textParam;
				if (textParam != null) {
					textFnArgs.push(textParam);
				}
				var text:String = centerPrintData.textFn.apply(null, textFnArgs);
				
				var centerPrintType:String = centerPrintData.type;
				
				this.winMap.addCenterPrint(text, centerPrintType);
			}
		}
		
		private var playSoundSoundTransform:SoundTransform = new SoundTransform();
		private function playSound(soundId:String, vol:Number = 1.0):void
		{
			const s:Sound = this.getSound(soundId);
			if (s)
			{
		...
		public function playHeartSound(hpPer:Number):void{
			...
			if (hpPer < 35 && hpPer > 20 && this.hbType != "Normal"){				
				this.hbSound = this.getSound("ui_heartbeat")//("heartbeat_normal");
				this.hbType = "Normal";
				start = true;
			...
			if (start){
				...
				var v:Number = (this._uiFramework.volumes.soundVolume/100) ;
				var t:SoundTransform = new SoundTransform(v);
				try{
		...
		// Generic Button Menu *****
		//***************************
		private var _gbms:Dictionary = new Dictionary(true);
		...
		public function openGenericButtonMenu(menuId:String, width:Number, text:String, btnLabels:Array, subText:String = null):void{
			...
			if(!w)
			{
				var gbm:UiGenericButtonMenu = new UiGenericButtonMenu(menuId, width, text, btnLabels, subText);
		...
		private function onGenericButtonMenuClick(evt:GlobalEvent):void{
			this._gateway.sendMsg( new BattleMessage(BattleEvent.UI_BUTTON_MENU, {menuId:evt.data.menuId, menuSelection:evt.data.option}) );
		...
		// return a instance of the AdminPanel
		private function getNewAdminPanel():UiAdminPanel{
			return new UiAdminPanel();
		}

		// return a instance of the accept decline message window 
		//this could eventualy be abstarcted out more to be abble to have custom errors message 
		//with OK/CANCLE or just OK or ACCEP/DECLINE.. by passing diferent parameters.
		...
		// if the user is a guest, pop this up all over the site
		...
		// return a instance of the OptionPanel
		...
		private function getSound(soundId:String):Sound {
			try{			
				var c:Class = this._uiFramework.assetFactory.getClass(soundId);
				var s:Sound = new c() as Sound;
			}catch(e:Error){
				trace(e.message);
			}
			return s;
		...
		private function onTextBasedUICommand(e:GlobalEvent):void {
			...
			var restParams:String = parameters[1];
			...
			switch(firstParam.toLowerCase()){				
				...
				case "lfg":
					...
					break;
				case "help":		
					var topicName:String = restParams;	
		...
		private function onActorLeave(evt:BattleEvent):void {
			...
			if (ActorManager.actorIdToActor(actorId) != null && (actorId != ActorManager.getInstance().myActor.actorId)){								
				//--- Close dialog if NPC is leaving and Dialog is still open
		...
		public static function formatConLevel(conLevel:Number):String {
			...
			var conLevelText:String = String(conLevelRounder);
		...
		public function onLinkClicked(e:GlobalEvent):void {
			...
			if (list != null){
				...
				var w:int = this.linksPanel.width + 15;
				var h:int = this.linksPanel.height + 10;
				if (w < 80) w = 80;
				if (h < 50) h = 50;			
		...
		private function onProgress(evt:ProgressEvent):void{			
			...
			//trace("[uiManager onProgress] ---------- ",this._progressManager.label, per, evt.bytesLoaded/evt.bytesTotal);
		...
		private function onProgressComplete(evt:Event):void{
			//trace("[uiManager onPregressComplete] ========= Progresse Done")
		...
		private function initZomg():void{
			...
			this._gateway.addEventListener(BattleEvent.GRANT_RING, onGrantRing, false, 0, true); // triggers collectibles update, too			
		...
		// Action Bar Event	************************
		//******************************************
		//-- Init Action bar /  Create FrameRate
		...
		private function onMenuItemClick(evt:ActionBarEvent):void{
			switch (evt.menuItemName.toLowerCase()){
				...
				case Menu.CMD_HELP.toLowerCase():
					this.openGameHelp();
					break;	
				case Menu.CMD_REGISTER.toLowerCase():		// called only if they're a guest user
					var e:UiEvents = new UiEvents(UiEvents.REGISTER, "menu");
					this.onRegister(e);
					break;
				case Menu.CMD_DISCONNECT.toLowerCase():
				case Menu.CMD_QUIT.toLowerCase():
					if (ActorManager.getInstance().myActor.isGuestUser()) {	// should be set by now
						// intercept and give them a chance to register, cancel or quit
		...
		private function onLfmCallBack(evt:BattleEvent):void{
			...
			for (var i:int = 0; i < evt.battleMessage.responseObj.length; i++){
				...
				var res:Object = evt.battleMessage.responseObj[i];				
				playerList = res.connedPlayerList;
				ringList = res.connedRingMap;
			}

			if (playerList) {			
				for (var pi:int = 0; pi < playerList.length; pi++){
					...
					for (var ri:int = 0; ri < playerList[pi].connedPlayerRingIDList.length; ri++){
		...
		private function onInventoryTabChange(evt:Event):void{			
			...
			var notConsTab:Boolean = (this.inventoryPanel.activeTab != UiInventory.TAB_CONSUMABLES);
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ALLOW_CONSUMABLE_USE, {allow:notConsTab, allowCure:notConsTab}));
		}
		...
		private function closeInventory(e:Event):void {
			if (this.winInventory && isWindowOpen(this.winInventory)){			
				...
				GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ALLOW_CONSUMABLE_USE, {allow:true, allowCure:true}));				
		...
		private function onInventoryRingSalvage(evt:UiEvents):void{
			...
			if (slot >= 0 && ActorManager.getInstance().myActor.rings[slot] != null && !ActorManager.getInstance().myActor.isSlotLock(slot)){
				...
				var param:Object = {ringSlot:slot, upgrade:"SALVAGE"};
				var msg:BattleMessage = new BattleMessage("changeRing", param);
				this._gateway.sendMsg(msg);
			}
		...
		private function onInventoryRingUpgrade(evt:UiEvents):void{	
			...
			if (slot >= 0 && ActorManager.getInstance().myActor.rings[slot] != null && !ActorManager.getInstance().myActor.isSlotLock(slot)){			
				this.ringInventory.ringUpgrade.disableButtons();				
				ActorManager.getInstance().myActor.rings[slot].isUpdating = true;				
				var param:Object = {ringSlot:slot, upgrade:"STEP"};
				var msg:BattleMessage = new BattleMessage("changeRing", param);
				this._gateway.sendMsg(msg);
			}
		...
		private function openInventory(activeTab:String = null):void {
			...
			if (this.inventoryPanel) {
				...
				if (!isInventoryOpen()) {
					GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ALERT, {type:AlertTypes.GENERIC_OPEN}));
					// Adding these logs because we're having a wierd problem where sometimes the inventory doesn't show up
				...
				//already done in activeTab setter
				//RingItemManager.getInstance().usable = (activeTab != UiInventory.TAB_RING);
				//ConsumableItemManager.getInstance().usable = (activeTab != UiInventory.TAB_CONSUMABLES);	
				
				var isNullChamber:Boolean = this._uiFramework.map.isNullChamber();				
				this.showRingUpgrade(isNullChamber);	
			}
		}
		...
		public function showRingUpgrade(visible:Boolean):void {
			...
			var invWidth:Number = visible ? 556 : 475;								
		...
		private function onGrantRing(evt:BattleEvent):void{
			...
			if (this.winMap != null){
				GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.ALERT, {type:AlertTypes.RING_GRANTED, textParam:r.ringName}));																																																	
		...
		private function onGameItemUpdate(evt:GlobalEvent):void{
			...
			for each (var item:Object in evt.data.items){				
		...
		private function onMinimapLoaded(module:Object):void
		{
			...
			if (this._pendingMinimap)
			{
				...
				var local:UiMapDisplay = createMapDisplay(this._uiFramework.tooltipManager);
				this._minimapManager.addView(local, true);
				
				var global:UiMapDisplay = createMapDisplay(this._uiFramework.tooltipManager);
				...
				// host them in our widget window, and add to the flex layer
				const title:String = "MAP";
		...
		// return a instance of the Help Text Panel
		...
		private function loadModules():void
		{
			...
			var classRef:Class;
			var instance:Object;
			for each (var module:XML in _configManger.moduleXMLList) {
				classRef = getDefinitionByName(module.name) as Class;
				instance = new classRef();
				var params:XML = new XML(String(module.params));				
...
internal class CenterPrintData {
	private var _textFn:Function;
	private var _type:String;
	
	public function CenterPrintData(textFn:Function, type:String = ""):void {
		this._textFn = textFn;
		this._type = type;
	}
