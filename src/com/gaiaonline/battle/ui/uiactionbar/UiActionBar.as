package com.gaiaonline.battle.ui.uiactionbar
{
	...
	public class UiActionBar extends UiActionBarFl implements ISettings	{				
		...
		public function UiActionBar(uiFramework:IUIFramework, gateway:BattleGateway, actorManager:ActorManager)
		{
			...
			// Default to closed; may get opened by other events on startup if we're part of a team
			setCrewOpen(false);
		...
		private function onBtnPDAClick(evt:Event):void{
			...
			this.dispatchEvent(new Event(ActionBarEvent.BTN_PDA_CLICK));	//@@ [kejk] remove me
		...
		public function setQuestNotification(v:Boolean):void{
			if (v){
				if (_pdaGlowFilter == null) {
					_pdaGlowFilter = new PulsatingGlowFilter(this.btnPDA, 0xFFCC00, 1,6,6,3);
				}
			}else{
				if (_pdaGlowFilter != null) {
					_pdaGlowFilter.stop();
		...
		public function updateHealth(currentValue:Number,maxValue:Number = 100):void{
			...
			this.healthMeter.healthMeterAnim.txt.text = currentValue;//Math.round(p*100);
		...
		public function updateStamina(currentValue:Number,maxValue:Number = 100):void{f
