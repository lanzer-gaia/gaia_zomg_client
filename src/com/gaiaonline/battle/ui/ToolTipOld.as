package com.gaiaonline.battle.ui
{
	...
	public class ToolTipOld	
	{
		...
		private var _objects:Object = new Dictionary(true);
		private var _lastMousedOverItem:DisplayObject;  // [kja] should arguably use a weak reference here, but this gets reset so often it probably doesn't matter
		...
		public function ToolTipOld(toolTipLayer:Sprite, delay:int = 500){
			...
			this.mcBack.graphics.lineStyle(0,0xCCCCCC);
			this.mcBack.graphics.beginFill(0xFFFFCC);
		...
		{
			const entry:TooltipEntry = this._objects[me.currentTarget];
			if (entry)
			{
		...
		private function onTimer(evt:TimerEvent):void
		{
			...
			const entry:TooltipEntry = this._objects[this._lastMousedOverItem];
