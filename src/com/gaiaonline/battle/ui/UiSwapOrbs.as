package com.gaiaonline.battle.ui
{
	...
	public class UiSwapOrbs extends MovieClip
	{
		...
		private var _rh1:Sprite = new Sprite();
		private var _rh2:Sprite = new Sprite();	
		...
		public function dropItemIcon(x:Number, y:Number, ic:ItemIcon):void{
			...
			if (this.mcSlots.slot1.hitTestPoint(x, y)){	
				...
				if (this._itemIcon1 != null){ this._itemIcon1.inSwapUi = false }
				...
				this.clearMc(this._rh2);
				if (this._itemIcon2 != null){ this._itemIcon2.inSwapUi = false }
			...
			if (this.mcClLevels.txtCl1.text == this.mcClLevels.txtCl2.text){
				TextField(this.mcClLevels.txtCl1).textColor = 0XFF0000;
				TextField(this.mcClLevels.txtCl2).textColor = 0XFF0000;
			}else{
				TextField(this.mcClLevels.txtCl1).textColor = 0XFFFFFF;
				TextField(this.mcClLevels.txtCl2).textColor = 0XFFFFFF;
			}
		...
		private function onConfirmClick(evt:MouseEvent):void{
			...
			//trace("[UiSwapOrbs] onConfirmClick ", this._itemIcon1.slot, this._itemIcon2.slot)			
			var msg:BattleMessage = new BattleMessage("orbSwap", {slot1:this._itemIcon1.slot , slot2:this._itemIcon2.slot});
		...
		private function onOrbSwap(evt:BattleEvent):void{
			...
			if (responseObj.success != null && responseObj.success == false){
				var dw:DialogWindow = new DialogWindow(this._uiFramework, this._linkManager, Globals.uiManager.mcDragLayer, 200);
 				dw.autoSize = true;
 				dw.keepOnStage = true;
 				var txt:String = responseObj.errorMessage as String;
		...
		private function onAnimDone(evt:Event):void{
			this.allowSwap = false;
			var ct:Number = getTimer();
			//this._nextValidTime = ct// + 86400000;
			this.setTimeText(this._nextValidTime - ct);
			if (this.stage != null){
				this._timer = new Timer(300);
				this._timer.addEventListener(TimerEvent.TIMER, onTimer, false, 0, true);
		...
		public function set allowSwap(v:Boolean):void{			
			if (v != this._allowSwap){				
				...
				this.mcSlots.alpha = this._allowSwap ? 1.0 : .3;
