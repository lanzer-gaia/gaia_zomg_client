package com.gaiaonline.battle.ui
{
	...
	public class WorldListManagerZomg extends EventDispatcher 
	{
		...
		private var _friendsList:Array = new Array();
		...
		public function WorldListManagerZomg(gateway:BattleGateway, baseUrl:String, serverPanelHolder:DisplayObjectContainer, 
			userSettings:UserSettings, partnerId:String, debugForceIP:Boolean = false){
			_gateway = gateway;
		...
		private function onServerListError(event:WorldListManagerEvent):void{
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.GENERIC_ERR_MSG, {header:"Server List Error", message:"Invalid Server List", reset:false}));
		}
		
		private function onServerListLoadError(event:WorldListManagerEvent):void{
			GlobalEvent.eventDispatcher.dispatchEvent(new GlobalEvent(GlobalEvent.GENERIC_ERR_MSG, {header:"Server List Error", message:event.data, reset:false}));
		}
		...
		{
			// [bgh] removed references to the vivox js code
		}
		...
		private function onVideoDone(event:GlobalEvent):void{
			var temp:IEventDispatcher; // just to keep the auto importer happy
		...
		public function showPanel():void{_worldListManager.showPanel();}
		public function set forceAutoconnectIP(ip:String):void{_worldListManager.forceAutoconnectIP = ip;}
		public function get ready():Boolean{return _worldListManager.ready;}
