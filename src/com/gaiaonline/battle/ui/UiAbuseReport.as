package com.gaiaonline.battle.ui
{
	...
	public class UiAbuseReport extends MovieClip
	{
		...
		public function UiAbuseReport(gateway:BattleGateway)
		{
			...
			TextField(this.mcReportInputs.lblReport).textColor = 0xFFFFFF;
			TextField(this.mcReportInputs.lblOffender).textColor = 0xFFFFFF;
			TextField(this.mcReportInputs.lblNote).textColor = 0xFFFFFF;
		...
		private function onAbuseReported(evt:GlobalEvent):void{
			var txt:String = "<p>Your report has been sent to Gaia Online's administrators and moderators.</p><br>";
		...
		private function validate():Boolean{
			...
			if (this.mcReportInputs.cbReport.selectedItem == undefined){
				TextField(this.mcReportInputs.lblReport).textColor = 0xFF0000;
				r = false;
			}else{
				TextField(this.mcReportInputs.lblReport).textColor = 0xFFFFFF;
			}
			...
			if (TextField(this.mcReportInputs.txtUserName).text.length == 0){
				TextField(this.mcReportInputs.lblOffender).textColor = 0xFF0000;
				r = false;
			}else{
				TextField(this.mcReportInputs.lblOffender).textColor = 0xFFFFFF;
			}
			...
			if (TextField(this.mcReportInputs.txtNote).text.length == 0){
				TextField(this.mcReportInputs.lblNote).textColor = 0xFF0000;
				r = false;
			}else{
				TextField(this.mcReportInputs.lblNote).textColor = 0xFFFFFF;
			}
