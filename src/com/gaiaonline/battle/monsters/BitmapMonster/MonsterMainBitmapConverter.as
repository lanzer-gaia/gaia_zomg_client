package com.gaiaonline.battle.monsters.BitmapMonster
{
	...
	{						
		private var _displayObjectStopper:DisplayObjectStopper = new DisplayObjectStopper(DisplayObjectStopperModes.SHOW_NO_ANIM, true);
		private var _matrix:Matrix = new Matrix();
		private var _rect:Rectangle = new Rectangle();
		private var _bmdRef:Object = new Object();
		...
		public function convert(mc:MovieClip):void{			
			...
			//-- Create the dataStructure and remove all state movieclip from display list
			var angles:Array = createDataStructure(mc);
			
			///--- replace state MovieClip with animationData in structure;
		...
		private function createDataStructure(mc:MovieClip):Array{
			...
			for (var angIndex:int = 0; angIndex < mc.numChildren; angIndex++){
				var mcAng:MovieClip = mc.getChildAt(angIndex) as MovieClip;
				var poses:Object = new Object();
				for (var poseIndex:int = 0; poseIndex < mcAng.numChildren; poseIndex++){
					var mcPose:MovieClip = mcAng.getChildAt(poseIndex) as MovieClip;
					var states:Object = new Object();
					var stateCount:int = mcPose.numChildren;					
					for (var stateIndex:int = 0; stateIndex < stateCount; stateIndex++){
						var mcState:MovieClip = mcPose.getChildAt(0) as MovieClip;										
						states[mcState.name] = mcState;
						mcPose.removeChild(mcState);
						this._states.push({angIndex:angIndex, pose:mcPose.name, state:mcState.name});
										
					}							
					poses[ MonsterPoseEnum(Enumeration.getEnumFromName(MonsterPoseEnum, mcPose.name)) ] = states;					
				}				
				angles.push({angMin:mcAng.angMin, angMax:mcAng.angMax, poses:poses})		
			}	
			return angles;				
		}
		...
		private function createAnimationsData(mc:MovieClip, angles:Array):void{
			...
			for (var i:int = this._currentStateIndex; i < this._states.length; i++){
				var angIndex:int = this._states[i].angIndex;
				var pose:String = this._states[i].pose;
				var state:MovieClip = angles[angIndex].poses[pose][this._states[i].state];
				
				//trace(i, angIndex, pose, state.name, state);
						///--- add the state movieClip in the displaylist						
						MovieClip(MovieClip(mc.getChildAt(angIndex)).getChildByName(pose)).addChild(state);
						
						//--- get animation data						
						var anims:Object = getAnimations(mc, state);
						
						//-- remove MovieCliop
						MovieClip(MovieClip(mc.getChildAt(angIndex)).getChildByName(pose)).removeChild(state);
																												
						//---replace the movieClip with the dxata in the Structure
		...
		private function getAnimations(topMc:MovieClip, mc:MovieClip):Object{
			...
			var skipCount:int = 0;
			var lastFrame:BitmapFrame;			
			for (var f:int = 1; f <= mc.totalFrames; f ++){					
				mc.gotoAndStop(f);					
				if (mc.currentLabel != null && mc.currentLabel.toLowerCase() != currentLabel && 
					(mc.currentLabel.toLowerCase().substr(0,2) == "ts" ||
					 mc.currentLabel.toLowerCase() == "qmin" ||
					 mc.currentLabel.toLowerCase() == "qmout" || 
					 mc.currentLabel.toLowerCase() == "qmloop" ||
					 mc.currentLabel.toLowerCase() == "end")){
		...
		private function onTimerComplete(evt:TimerEvent):void{	
			//trace("AAAAAAA", this._currentStateIndex);		
		...
		private function getBitmapFrame(topMc:MovieClip, mc:MovieClip, frameIndex:int):BitmapFrame{
			...
			if (topMc.width > 0 && topMc.height > 0){								
				...
				bmd  = new BitmapData(Math.max(this._rect.width * this._precision,1), Math.max(this._rect.height * this._precision,1), true, 0x00000000);
				...
				if (this._bmdRef[ba] != null){					
					...
					this._totalMem += (bmd.width * bmd.height * 4);	
					this._bmdRef[ba] = bmd;
				}
				//this._totalMem += (bmd.width * bmd.height * 4);	
